...
void init_vectors(double* x, double* x_bar, struct dual_vector_2d proximated, struct dual_vector_2d grad_u, gray_img* src_image, int M, int N) {
	for (int i = 0; i < M; i++) {
		for (int j = 0; j < N; j++) {
...
void free_memory_of_vectors(double* x, double* x_bar, double* x_current, double* divergence, double* energy, struct dual_vector_2d y, struct dual_vector_2d proximated, struct dual_vector_2d grad_u) {
	free(x);
	free(x_bar);
	free(x_current);
	free(divergence);
...
void primal_dual_algorithm(gray_img* src, void (*prox_f)(struct dual_vector_2d, struct dual_vector_2d, struct parameter*, int, int), void (*prox_g)(double*, double*, gray_img*, struct parameter*), struct parameter* input_parameter, int spacing, int update, int steps, const char* filename) {
	...
	struct dual_vector_2d grad_u = {alloc_double_array(M, N), alloc_double_array(M, N)};
	...
	double* energy = alloc_double_array(steps+1, 1);

	init_vectors(x, x_bar, proximated, grad_u, src, M, N);

	gradient_of_image_value(grad_u, 1.0, x_bar, M, N, 0);
	double normalization_value = computed_energy_of_functional(src, grad_u, x_bar, input_parameter, M, N);
	energy[0] = 1.0;
	...
	for (int k = 1; k <= steps; k++) {
		...
		gradient_of_image_value(grad_u, 1.0, x_bar, M, N, 0);
		energy[k] = computed_energy_of_functional(src, grad_u, x_bar, input_parameter, M, N) / normalization_value;
		if (k > 200) {
			double test_value = energy[k - 1];
			if (test_value - energy[k] < 0.000001) {
				printf("%d\n", k);
				printf("%f\n", energy[k] * normalization_value);
	...
	solution_to_file(energy, steps, filename);

	free_memory_of_vectors(x, x_bar, x_current, divergence, energy, y, proximated, grad_u);
}