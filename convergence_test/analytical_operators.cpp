...
double isotropic_total_variation_norm(struct dual_vector_2d y, int M, int N) {
	double norm = 0.0;
	for (int i = 0; i < M; i++) {
		for (int j = 0; j < N; j++) {
			norm += sqrt(pow(y.x1[j + i * N], 2) + pow(y.x2[j + i * N], 2));
		}
	}
	return norm;
}

double standard_squared_l2_norm(gray_img* src_image, double* x_bar, struct parameter* input_parameter, int M, int N) {
	double norm = 0.0;
	double lambda_over_two = input_parameter->lambda/2.0;
	for (int i = 0; i < M; i++) {
		for (int j = 0; j < N; j++) {
			// norm += (lambda_over_two * pow(x_bar[j + i * N] - src_image->image_data[j + i * N], 2));
			norm += (lambda_over_two * fabs(x_bar[j + i * N] - src_image->image_data[j + i * N]));
		}
	}
	return norm;
}

double computed_energy_of_functional(gray_img* src_image, struct dual_vector_2d y, double* x_bar, struct parameter* input_parameter, int M, int N) {
	// double h2 = 1.0/M * 1.0/N;
	return (isotropic_total_variation_norm(y, M, N) + standard_squared_l2_norm(src_image, x_bar, input_parameter, M, N))/(M*N);
}

void solution_to_file(double* energy, int steps, const char* filename) {
    FILE *file;
