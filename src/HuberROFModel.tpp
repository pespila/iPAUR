...
template<typename aType>
void HuberROFModel<aType>::SetSolution(WriteableImage<aType>& dst) {
	for (int k = 0; k < channel; k++)
		for (int i = 0; i < height; i++)
			for (int j = 0; j < width; j++)
...
template<typename aType>
void HuberROFModel<aType>::HuberROF(Image<aType>& src, WriteableImage<aType>& dst, Parameter<aType>& par) {
