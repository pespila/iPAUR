#include "dilatation.h"

void dilatation(Image& src, WriteableImage& dst, int* filter, int radius) {
    unsigned short height = src.get_height();
    unsigned short width = src.get_width();
    dst.reset_image(height, width, src.get_type());
    unsigned short sup = 0;
    int size = height * width;
    int channel = src.get_channels();
    int x = 0, y = 0;
    int i, j, k, l;

    short** filtered = (short**)malloc(size*sizeof(short*));
    for (i = 0; i < size; i++) filtered[i] = (short*)malloc(channel*sizeof(short));

    for (l = 0; l < channel; l++) {
        for (i = 0; i < height; i++) {
            for (j = 0; j < width; j++) {
                sup = 0;
                y = width - 1;
                for (k = (-1) * radius; k <= radius; k++) {
                    x = j + k;
                    if (x < 0) {
                        if (filter[k + radius] == 1 && src.get_pixel(i, 0, l) > sup) {
                            sup = src.get_pixel(i, 0, l);
                        }
                    } else if (x >= width) {
                        if (filter[k + radius] == 1 && src.get_pixel(i, y, l) > sup) {
                            sup = src.get_pixel(i, y, l);
                        }
                    } else {
                        if (filter[k + radius] == 1 && src.get_pixel(i, x, l) > sup) {
                            sup = src.get_pixel(i, x, l);
                        }
                    }
                }
                filtered[j + i * width][l] = sup;
            }
        }
    }

    for (l = 0; l < channel; l++) {
        dst.set_c_channel(l);
        for (i = 0; i < height; i++) {
            for (j = 0; j < width; j++) {
                sup = filtered[j + i * width][l];
                y = height - 1;
                for (k = (-1) * radius; k <= radius; k++) {
                    x = i + k;
                    if (x < 0) {
                        if (filter[k + radius] == 1 && filtered[j][l] > sup) {
                            sup = filtered[j][l];
                        }
                    } else if (x >= height) {
                        if (filter[k + radius] == 1 && filtered[j + y * width][l] > sup) {
                            sup = filtered[j + y * width][l];
                        }
                    } else {
                        if (filter[k + radius] == 1 && filtered[j + x * width][l] > sup) {
                            sup = filtered[j + x * width][l];
                        }
                    }
                }
                dst.set_pixel(i, j, sup);
            }
        }
    }

    for (i = 0; i < size; i++) free(filtered[i]);
    free(filtered);
}