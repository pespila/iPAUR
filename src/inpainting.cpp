...
void laplace_operator(float* y1, float* y2, float* image, int M, int N) {
	float h_M = 1.0/M;
	float h_N = 1.0/N;
	for (int i = 0; i < M; i++) {
		for (int j = 0; j < N; j++) {
			y1[j + i * N] = i + 1 < M && i - 1 >= 0 ? (image[j + (i+1) * N] - 2.0 * image[j + i * N] + image[j + (i-1) * N]) / h_M : 0.0;
			y2[j + i * N] = j + 1 < N && j - 1 >= 0 ? (image[j + 1 + i * N] - 2.0 * image[j + i * N] - image[j - 1 + i * N]) / h_N : 0.0;
		}
	}
}

void identity(float* y1, float* y2, float* image, int M, int N) {
	for (int i = 0; i < M; i++) {
		for (int j = 0; j < N; j++) {
			y1[j + i * N] = image[j + i * N];
			y2[j + i * N] = image[j + i * N];
		}
	}
}
...
void image_inpainting_color(gray_img* src, param* parameter, const char* filename, int steps) {
	const int M = src->image_height;
	const int N = src->image_width;
	const int size = M*N;
	double stopping_criterion = 1e-4;
