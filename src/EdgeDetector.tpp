...
EdgeDetector<aType>::EdgeDetector(int height, int width, char type) {
    this->height = height;
    this->width = width;
    this->type = type;
...
template<typename aType>
void EdgeDetector<aType>::Hysteresis(WriteableImage<aType>& dst, aType* nms, const int TL, const int TH) {
    int i, j, k, l, x, y;
    for (i = 0; i < height; i++) {
        for (j = 0; j < width; j++) {
            if (nms[j + i * width] > TH) {
...
template<typename aType>
void EdgeDetector<aType>::SetEdges(WriteableImage<aType>& dst, aType* gx, aType* gy) {
    int i, j, sum = 0;
    for (i = 0; i < height; i++) {
        for (j = 0; j < width; j++) {
...
template<typename aType>
void EdgeDetector<aType>::Sobel(Image<aType>& src, WriteableImage<aType>& dst) {
...
template<typename aType>
void EdgeDetector<aType>::Prewitt(Image<aType>& src, WriteableImage<aType>& dst) {
    dst.Reset(this->height, this->width, this->type);
    NablaX(gx, src, 3);
    NablaY(gy, src, 3);
    SetEdges(dst, gx, gy);
...
template<typename aType>
void EdgeDetector<aType>::RobertsCross(Image<aType>& src, WriteableImage<aType>& dst) {
    dst.Reset(this->height, this->width, this->type);
...
template<typename aType>
void EdgeDetector<aType>::Laplace(Image<aType>& src, WriteableImage<aType>& dst) {
    dst.Reset(this->height, this->width, this->type);
    int i, j, v, w, x, y, sum;
    for (i = 0; i < height; i++) {
...
template<typename aType>
void EdgeDetector<aType>::Canny(Image<aType>& src, WriteableImage<aType>& dst, const int TL, const int TH) {
    dst.Reset(this->height, this->width, this->type);
    int i;
