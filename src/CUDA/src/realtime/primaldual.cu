...
__global__ void primal_descent(float* y1, float* y2, float* xbar, float sigma, float lambda, float alpha, int w, int h, int nc)
{
    int x = threadIdx.x + blockDim.x * blockIdx.x;
    int y = threadIdx.y + blockDim.y * blockIdx.y;
    int z = threadIdx.z + blockDim.z * blockIdx.z;
    ...
    if (x < w && y < h && z < nc)
    {
        ...
        // float factor = (2.0 * alpha) / (sigma + 2.f * alpha);
        // float bound = (lambda / alpha) * sigma * (sigma + 2.f * alpha);
        float factor = 1.f;
        float bound = 2.f * lambda * sigma;
        float norm = l2Norm(x1, x2)*l2Norm(x1, x2);
...
int main(int argc, char **argv)
{
    ...
    {
        primal_descent <<<grid, block>>> (d_y1, d_y2, d_xbar, sigma, lambda, alpha, w, h, nc);
