...
ImageInpainting::ImageInpainting(Image& src, int steps) {
	this->steps = steps;
...
void ImageInpainting::Initialize(Image& src) {
...
void ImageInpainting::SetSolution(WriteableImage& dst) {
	for (int k = 0; k < channel; k++)
		for (int i = 0; i < height; i++)
			for (int j = 0; j < width; j++)
...
void ImageInpainting::Nabla(float* gradient_x, float* gradient_y, float* u_bar) {
	for (int k = 0; k < channel; k++) {
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
...
void ImageInpainting::ProxRstar(float* p_x, float* p_y, float* p_tilde_x, float* p_tilde_y, float alpha, float sigma) {
...
void ImageInpainting::NablaTranspose(float* gradient_transpose, float* p_x, float* p_y) {
	float x = 0.0;
...
void ImageInpainting::ProxD(float* u, float* u_tilde, float* f, unsigned char* hash_table, float tau, float lambda) {
	int small_size = height*width;
	for (int k = 0; k < channel; k++) {
		for (int i = 0; i < small_size; i++) {
...
void ImageInpainting::Inpaint(Image& src, WriteableImage& dst, Parameter& par) {
	int i;
