...
template<typename aType>
void HuberROFModel<aType>::DualAscent(aType* p_x, aType* p_y, aType* u_bar, aType alpha, aType sigma) {
	int I;
	aType u1, u2, norm;
	aType fac = 1.f + sigma * alpha;
	for (int k = 0; k < channel; k++) {
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				...
				p_x[I] = (u1/fac)/fmax(1.f, (norm/fac));
				p_y[I] = (u2/fac)/fmax(1.f, (norm/fac));
			}
		}
	}
}
...
template<typename aType>
aType HuberROFModel<aType>::PrimalDescent(aType* u_bar, aType* u, aType* p_x, aType* p_y, aType lambda, aType tau, aType theta) {
	int I;
...
template<typename aType>
void HuberROFModel<aType>::HuberROF(Image<aType>& src, Image<aType>& dst, aType lambda, aType alpha, aType tau) {
	int k;
	aType nrj, nrjn = 0.f;
	...
	Initialize(src);
	for (k = 1; k < steps; k++) {
		DualAscent(p_x, p_y, u_bar, alpha, sigma);
