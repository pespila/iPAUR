...
HuberROFModel<aType>::HuberROFModel(Image<aType>& src, int steps) {
	this->steps = steps;
...
template<typename aType>
void HuberROFModel<aType>::Initialize(Image<aType>& src) {
...
template<typename aType>
void HuberROFModel<aType>::SetSolution(Image<aType>& dst) {
	for (int k = 0; k < channel; k++)
		for (int i = 0; i < height; i++)
			for (int j = 0; j < width; j++)
...
template<typename aType>
void HuberROFModel<aType>::HuberROF(Image<aType>& src, Image<aType>& dst, aType alpha, aType lambda, aType tau) {
	int k;
