...
template<typename aType>
void TVL1Model<aType>::DualAscent(aType* p_x, aType* p_y, aType* u_bar, aType sigma) {
	int I;
	aType u1, u2, norm;
	for (int k = 0; k < channel; k++) {
		for (int i = 0; i < height; i++) {
...
template<typename aType>
aType TVL1Model<aType>::PrimalDescent(aType* u_bar, aType* u, aType* p_x, aType* p_y, aType lambda, aType tau, aType theta) {
	int I;
	aType fac = tau*lambda;
	aType u1, u2, un, val, img, norm = 0.f;
	for (int k = 0; k < channel; k++) {
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				I = j + i * width + k * height * width;
				...
				val = un + tau * (u1+u2);
				img = val - f[I];
				if (img > fac) 			u[I] = val - fac;
				if (img < -fac) 		u[I] = val + fac;
				if (fabs(img) <= fac) 	u[I] = f[I];
