...
URModel<aType>::URModel(Image<aType>& src, int steps) {
	this->steps = steps;
...
template<typename aType>
void URModel<aType>::Initialize(Image<aType>& src) {
...
template<typename aType>
void URModel<aType>::SetSolution(Image<aType>& dst) {
	for (int k = 0; k < channel; k++)
		for (int i = 0; i < height; i++)
			for (int j = 0; j < width; j++)
...
template<typename aType>
void URModel<aType>::Nabla(aType* gradient_x, aType* gradient_y, aType* u_bar, aType* p_x, aType* p_y, aType sigma) {
	for (int k = 0; k < channel; k++) {
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
...
template<typename aType>
void URModel<aType>::ProxRstar(aType* p_x, aType* p_y, aType* p_tilde_x, aType* p_tilde_y) {
	aType vector_norm = 0;
	for (int k = 0; k < channel; k++) {
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
...
template<typename aType>
void URModel<aType>::NablaTranspose(aType* gradient_transpose, aType* p_x, aType* p_y, aType* u_n, aType tau) {
	aType x = 0;
	aType x_minus_one = 0;
	for (int k = 0; k < channel; k++) {
		for (int i = 0; i < height; i++) {
...
template<typename aType>
void URModel<aType>::ProxD(aType* u, aType* u_tilde, aType* f, aType tau, aType alpha, aType beta) {
...
template<typename aType>
void URModel<aType>::Extrapolation(aType* u_bar, aType* u, aType* u_n, aType theta) {
	for (int i = 0; i < size; i++)
	{
...
template<typename aType>
aType URModel<aType>::PrimalEnergy(aType* u, aType* g, aType alpha, aType beta) {
	aType energy = 0;
	aType dx = 0;
	aType dy = 0;
	for (int k = 0; k < channel; k++) {
...
template<typename aType>
aType URModel<aType>::StopCriterion(aType* u, aType* u_n) {
	aType norm = 0;
	for (int k = 0; k < channel; k++) {
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
...
template<typename aType>
void URModel<aType>::UR(Image<aType>& src, Image<aType>& dst, aType alpha, aType beta, aType tau) {
	int k;
	aType theta = 1;
	aType sigma = (aType)1 / (aType)(tau * 8);
	aType energy = PrimalEnergy(u, f, alpha, beta);
