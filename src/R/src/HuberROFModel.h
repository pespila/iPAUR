...
public:
	...
	HuberROFModel(const NumericMatrix&, int, int, int, int);
	~HuberROFModel();

	void HuberROF(const NumericMatrix&, NumericMatrix&, aType, aType, aType);
};

template<typename aType>
HuberROFModel<aType>::HuberROFModel(const NumericMatrix& src, int steps, int height, int width, int channel) {
	this->steps = steps;
...
template<typename aType>
HuberROFModel<aType>::~HuberROFModel() {
	free(f);
	free(u);
	free(u_n);
	free(u_bar);
...
template<typename aType>
void HuberROFModel<aType>::Initialize(const NumericMatrix& src) {
...
template<typename aType>
void HuberROFModel<aType>::SetSolution(NumericMatrix& dst) {
	for (int k = 0; k < channel; k++)
		for (int i = 0; i < height; i++)
			for (int j = 0; j < width; j++)
...
template<typename aType>
void HuberROFModel<aType>::Nabla(aType* gradient_x, aType* gradient_y, aType* u_bar, aType* p_x, aType* p_y, aType sigma) {
	for (int k = 0; k < channel; k++) {
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
...
template<typename aType>
void HuberROFModel<aType>::ProxRstar(aType* p_x, aType* p_y, aType* p_tilde_x, aType* p_tilde_y, aType alpha, aType sigma) {
...
template<typename aType>
void HuberROFModel<aType>::NablaTranspose(aType* gradient_transpose, aType* p_x, aType* p_y, aType* u_n, aType tau) {
	aType x = 0.0;
...
template<typename aType>
void HuberROFModel<aType>::ProxD(aType* u, aType* u_tilde, aType* f, aType tau, aType lambda) {
	for (int i = 0; i < size; i++)
		u[i] = (u_tilde[i] + tau * lambda * f[i]) / (1.0 + tau * lambda);
}

template<typename aType>
void HuberROFModel<aType>::Extrapolation(aType* u_bar, aType* u, aType* u_n, aType theta) {
	for (int i = 0; i < size; i++)
	{
...
template<typename aType>
void HuberROFModel<aType>::HuberROF(const NumericMatrix& src, NumericMatrix& dst, aType alpha, aType lambda, aType tau) {
	Initialize(src);
	...
	for (int k = 0; k < steps; k++)
	{
		...
		ProxRstar(p_x, p_y, gradient_x, gradient_y, alpha, sigma);
