...
void huber_rof_model(gray_img* src, param* parameter, const char* filename, int steps) {
	...
	for (k = 1; k <= steps; k++) {
		...
		proximation_g(x, divergence, image, parameter->tau, parameter->lambda/2.0, size);
...
void huber_rof_model_color(gray_img* src, param* parameter, const char* filename, int steps) {
	const int M = src->image_height;
	const int N = src->image_width;
	const int size = M*N;
	double stopping_criterion = 1e-5;
